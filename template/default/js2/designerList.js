...
function changeImg(){
    ...
    prodDesiItem.each(function(index,ele){
        str1 = "prodDesc";
        $(ele).find("img").attr("src","/template/default/img/" + str1 + (index + 1) +".jpg");
    });
    picDesiItem.each(function(index,ele){
        str2 = "picDesc";
        $(ele).find("img").attr("src","/template/default/img/" + str2 + (index + 1) + ".jpg");
    });
}
function itemHover(){
    var $this,parentId;
...
function maskShow(str,$this){
    ...
    $this.find("img").attr("src","/template/default/img/" + str + dataIndex + ".jpg");
}
function allShow(str,$this){
    var dataIndex = $this.attr("data-index");
    $this.find("img").attr("src","/template/default/img/" + str + dataIndex + "_1.jpg");
}
